...
/* ═══ ٦ · قيمة اللتر ═══ */
{
  ...
  band(s, 1.34, "هامش المساهمة " + ar2(L.cpl_margin) + " هللة — أساس كل قرار حدّي  ·  وبعد تشغيل المحطة " + ar2(L.cpl_net) + "  ·  وبعد محمّلات المركز " + ar2(L.cpl_company)
    + "  —  ولكل قرارٍ أساسه، وخلطها خطأ");
  ...
  const cards = [[ar(Math.round(L.box * 100)) + " هللة", "كلفة العلبة", "مؤكَّدة من الإدارة", ORANGE],
                 [ar1(L.breakeven_extra) + " لتر", "تعادل الهدية",
                  ar1(L.breakeven_sar) + " ريالاً إنفاقاً إضافياً", GOOD],
                 [ar2(SEG[2].net_txn) + " ريالاً", "مساهمة المعاملة الإضافية",
                  "وهي سقف أي مكافأة — لا الصافي بعد تكلفة ثابتة", GOOD]];
  cards.forEach((c, i) => {
