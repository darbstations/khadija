...
/* ═══ ٢ · الخلاصة ═══ */
{
  ...
  const dec = [["اعتماد المستهدف بمسارين",
...
/* ═══ ٦ · قيمة اللتر ═══ */
{
  ...
  band(s, 1.34, "العتبة بالريال والربح بالهللة — والصرف بينهما سعرُ المضخة  ·  فهامش لتر البنزين " + ar2(L.cpl_petrol) + " هللة ولتر الديزل " + ar2(L.cpl_diesel)
    + "، والفاتورة الواحدة لا تُساوي نفسها");
  ...
  const rows = L.rows.map((r, i) => ({
    c: [{ t: r.fuel, a: "right", b: true }, ar2(r.price), ar2(r.net_price),
        { t: ar2(r.litres), b: true }, ar2(r.cpl),
  ...
  rows.push({ c: [{ t: "المرجّح", b: true }, ar2(50 / L.w_litres), ar2(50 / L.w_litres / 1.15),
                  ar2(L.w_litres), ar2(L.cpl_margin), ar2(L.w_net), ar2(L.w_net_station),
                  ar2(L.box), ar2(L.w_net - L.box), pc0(L.box / L.w_net)],
              fill: T_BAND, b: true });
  table(s, M, 2.42, CW, [
    { t: "المنتج", w: 11, a: "right" }, { t: "سعر المضخة", w: 10 },
    { t: "صافي الضريبة", w: 10 }, { t: "لتر بـ٥٠ ريالاً", w: 11 },
    { t: "هللة لكل لتر", w: 10 }, { t: "مساهمة التعبئة", w: 11 },
    { t: "بعد تشغيل المحطة", w: 11 }, { t: "علبة المناديل", w: 9 },
    { t: "المساهمة بعد الهدية", w: 10 }, { t: "الهدية من المساهمة", w: 11 }],
    rows, { rh: 0.4, fs: 10.5, hfs: 8.5 });
  const cw = (CW - 2 * 0.2) / 3;
  const cards = [[ar2(L.rows[0].net) + " مقابل " + ar2(L.rows[2].net), "مساهمة فاتورة الخمسين",
                  "بنزين ٩١ مقابل ديزل — الفاتورة نفسها والفارق " +
                  pc0(L.rows[0].net / L.rows[2].net - 1), BAD],
                 [ar1(L.breakeven_extra) + " لتر بنزين", "تعادل الهدية",
                  "ومن الديزل " + ar1(L.breakeven_diesel) + " لتراً — ثلاثة أضعاف", GOLD],
                 [pc0(L.box / L.rows[2].net), "الهدية من فاتورة ديزل",
                  "مقابل " + pc0(L.box / L.rows[0].net) + " من فاتورة بنزين", BAD]];
  cards.forEach((c, i) => {
  ...
  foot(s, "هامش المنتجين من انحدار ١٩ محطة على حصة ديزلها (R²=٠٫٥٦) ويسنده هيكل عمولتنا "
  + "٣ هللات بنزين و١٫٥ ديزل · وأقصى حصة ديزل مرصودة ٤٣٪ فرقم الديزل استقراء لا قياس");
}
...
/* ═══ أين تُخسر المحطة ═══ */
{
  const s = page("أين تُخسر المحطة — ليس على الوقود",
                 "١٨ محطة خاسرة بـ٣٬٦٩١ ألف ريال · من تقرير المالية يوليو ٢٠٢٦ (ص١١)");
  const cw = (CW - 3 * 0.18) / 4;
  const st = [["١٨", "محطة خاسرة", "من ١٨٩ قائمة دخل محطة", BAD],
              ["١١", "منها إيجاري", "تحمل ٦٨٪ من الخسارة", BAD],
              ["٦٫٠٦٪", "هامش وقود الخاسرة", "وسيط — والرابحة ٥٫٩٤٪", GOOD],
              ["٣٬٦٩١", "ألف ريال خسارة", "ومنها ٢٬٤٩٦ من الإيجاري", BGRAY]];
  st.forEach((v, i) => stat(s, rtlx(i, cw, 0.18), 1.32, cw, 1.46, v[0], v[1], v[3], v[2]));

  head(s, 3.0, "① هامش الوقود لا يفرّق بين رابحة وخاسرة — فالخسارة ليست فيه");
  const rows = [
    { c: [{ t: "أعلى عشر محطات ربحاً", a: "right", b: true }, "١٠",
          { t: "٥٫٩٤٪", b: true, c: D_GOOD }, "٤٫١٦٪ إلى ٦٫٩١٪",
          { t: "+١٢٬٢٨٢ ألف", b: true, c: D_GOOD },
          "العوالي MK003 وحدها ٥٬٠٥٢ ألفاً بهامش صافٍ ١٢٫٥٢٪"], fill: T_GOOD },
    { c: [{ t: "المحطات الخاسرة — محطاتنا", a: "right", b: true }, "١٢",
          { t: "٦٫٠٦٪", b: true, c: D_BAD }, "٣٫٩٧٪ إلى ٦٫٦٨٪",
          { t: "−٢٬٥٧٧ ألف", b: true, c: D_BAD },
          "عشر من اثنتي عشرة هامشها فوق ٥٪ — وكلها تخسر"], fill: T_BAD }];
  table(s, M, 3.34, CW, [
    { t: "المجموعة", w: 22, a: "right" }, { t: "محطات", w: 8 },
    { t: "وسيط هامش الوقود", w: 12 }, { t: "المدى", w: 14 },
    { t: "صافي الدخل", w: 12 }, { t: "الشاهد", w: 32, a: "right" }],
    rows, { rh: 0.5, fs: 11, hfs: 10 });

  head(s, 4.9, "② بل في عقد الإيجار — والتقرير يقولها صراحةً");
  const cw2 = (CW - 0.24) / 2;
  const bx = [rtlx(0, cw2, 0.24), rtlx(1, cw2, 0.24)];
  s.addShape(p.ShapeType.roundRect, { x: bx[0], y: 5.24, w: cw2, h: 1.38, rectRadius: 0.05,
    fill: { color: T_BAD }, line: { color: BAD, width: 1.2 } });
  s.addText("النموذج الإيجاري — الحساب", { x: bx[0] + 0.16, y: 5.32, w: cw2 - 0.32, h: 0.3,
    fontFace: F, fontSize: 13, bold: true, color: D_BAD, ...rtl });
  const lines = [["الدخل الإيجاري", "+١٠٬٨٣٤"],
                 ["إهلاك حق الاستخدام", "−٨٬٨٧٤"],
                 ["تكلفة التمويل", "−٨٬١٦٥"],
                 ["= مساهمة الإيجار", "−٧٬١٤١ ألف ريال"]];
  lines.forEach((k, i) => {
    const y = 5.66 + i * 0.235;
    s.addText(k[0], { x: bx[0] + 0.2, y, w: cw2 * 0.6, h: 0.24, fontFace: F,
      fontSize: 11, bold: i === 3, color: i === 3 ? D_BAD : INK, ...rtl });
    s.addText(k[1], { x: bx[0] + cw2 - 1.9, y, w: 1.7, h: 0.24, fontFace: F,
      fontSize: 11, bold: i === 3, color: i === 3 ? D_BAD : INK,
      align: "left", margin: 0 });
  });
  s.addShape(p.ShapeType.roundRect, { x: bx[1], y: 5.24, w: cw2, h: 1.38, rectRadius: 0.05,
    fill: { color: T_OR }, line: { color: ORANGE, width: 1.2 } });
  s.addText("وما يترتّب على ذلك", { x: bx[1] + 0.16, y: 5.32, w: cw2 - 0.32, h: 0.3,
    fontFace: F, fontSize: 13, bold: true, color: ORANGE, ...rtl });
  s.addText("محطةٌ هامش وقودها ٦٫٦٨٪ وتخسر ١٫٨٠٪ لا تحتاج حملة ولا لتراً إضافياً — "
    + "تحتاج إعادة تفاوض على عقدها أو خروجاً منه. وكل ريال يُنفق عليها تسويقياً "
    + "يُنفق على مشكلة ليست تسويقية.",
    { x: bx[1] + 0.16, y: 5.66, w: cw2 - 0.32, h: 0.9, fontFace: F, fontSize: 11,
      color: INK, ...rtl });
  foot(s, "المعيصم MK002 شاهدٌ مزدوج: هامشه ٦٫٦٨٪ — الأعلى بين الخاسرة — وصافيه −١٫٨٠٪ · "
          + "وقد شخّصناه تغييرَ مدخلٍ لا منافسة، فالحجم لا يكفي ثوابته");
}
...
/* ═══ ربحية محطات التشغيل ═══ */
{
  ...
  const st = [["٩٧٫٠", "مليون ريال مبيعات", "+٨٫٥٪ عن الربع الأول", ORANGE],
              ...
              ["٣٫٤٢", "مليون ريال صافي", "بكلفة التقرير — وبكلفة القائمة يصير سالباً", BAD],
              ["٣٫٢٧٪", "هامش صافي الربح", "على أساسٍ يُنقص كلفة العمالة الثلثين", BAD],
              ["٥٠٨", "عاملاً", "−٥ · ربح العامل +٦٫٧٪", GOOD]];
  st.forEach((v, i) => stat(s, rtlx(i, cw, 0.16), 1.36, cw, 1.5, v[0], v[1], v[3], v[2]));
  head(s, 3.06, "وهذا التقرير لا يصلح مرجعاً للكلفة — قائمة الدخل تقول غير ذلك للمحطات نفسها");
  const rows = [
    { c: [{ t: "هامش المساهمة", a: "right", b: true }, "١١٫٠٩ هللة", "١١٫٤٤ هللة",
          { t: "+٠٫٣٥", c: D_GOOD }, "متقاربان — لا خلاف على الهامش"] },
    { c: [{ t: "مصاريف تشغيل المحطة", a: "right", b: true },
          { t: "٤٫٨٦ هللة", b: true, c: D_BAD }, { t: "٩٫١٥ هللة", b: true, c: D_GOOD },
          { t: "٢٫٠٢ ضعفاً", b: true, c: D_BAD },
          "التقرير يسعّر الأجر ٣٬٥٠٠ ر «للربع» — وهو أجر شهري طُبِّق مرة بدل ثلاث"],
      fill: T_BAD },
    { c: [{ t: "الصافي بعد التشغيل", a: "right", b: true }, "٦٫٢٣ هللة",
          { t: "٢٫٢٨ هللة", b: true, c: D_GOOD }, { t: "٢٫٧ ضعفاً تفاؤلاً", b: true, c: D_BAD },
          "والقائمة المالية هي المرجع"], fill: T_NEU }];
  table(s, M, 3.4, CW * 0.68, [
    { t: "البند", w: 20, a: "right" }, { t: "تقرير الربع الثاني", w: 14 },
    { t: "قائمة الدخل", w: 13 }, { t: "الفارق", w: 13 },
    { t: "لماذا", w: 40, a: "right" }], rows, { rh: 0.5, fs: 10.5, hfs: 9.5 });
  const x2 = M + CW * 0.71, w2 = CW * 0.29;
  s.addShape(p.ShapeType.roundRect, { x: x2, y: 3.4, w: w2, h: 1.84, rectRadius: 0.05,
    fill: { color: T_OR }, line: { color: ORANGE, width: 1.2 } });
  s.addText("ويبقى صالحاً لشيء واحد", { x: x2 + 0.14, y: 3.5, w: w2 - 0.28, h: 0.3, fontFace: F,
    fontSize: 13, bold: true, color: ORANGE, ...rtl });
  s.addText("المقارنة النسبية بين المحطات: كلفة العمالة لكل لتر تمتد من ٢٫٥٥ هللة في الجمعية إلى ٨٫٧٩ في حائل العريفي — ٣٫٤ أضعاف. الترتيب صحيح وإن كان المستوى ناقصاً.",
...
/* ═══ ١٣ · الشرائح ═══ */
{
  const s = page("خمسة تصنيفات — من سجلّ الشركة", "حيوية · خط سفر · حي · مختلط · نائية — ومتماسكة سلوكياً لا اسماً");
  ...
  table(s, M, 1.36, CW, [
    { t: "التصنيف", w: 15, a: "right" }, { t: "Site Type", w: 13 },
...
/* ═══ ١٥ · القسم الرابع ═══ */
divider("القسم الرابع", "خطة المبيعات", "عدد التعبئات أولاً · ثم حجم التعبئة");

/* ═══ ١٦ · المنهج ═══ */
{
  const s = page("المنهج — من أين يأتي النمو", "عدد التعبئات يُطلَب · وحجم التعبئة يُفسَّر ولا يُطلَب");
  band(s, 1.32, "الحجم  =  المعاملات  ×  متوسط حجم التعبئة        ←  الأولى تشغيلية، والثانية محكومة بمزيج الوقود");
  const cw = (CW - 0.24) / 2;
  const lev = [
    ["أولاً · عدد التعبئات", ar(Math.round(T.gap_peak)) + " معاملة/يوم",
     ar(Math.round(T.sar_txn / 1000)) + " ألف ريال", GOOD,
     ["نقارن شكل ساعات المحطة بشكل تصنيفها",
      "نحتسب النقص في ساعات ذروتها وحدها",
      "النقص في ساعات الهدوء اختلاف طلب لا فاقد",
      "العلاج: طاقم الذروة وتوفّر المضخات"]],
    ["ثانياً · حجم التعبئة الواحدة", ar(Math.round(T.upl_fill)) + " لتر/يوم",
  ...
  [[ar1(T.sar / 1e6), "مليون ريال — الفرصة المؤكَّدة", GOOD],
   [ar2(L.cpl_margin), "هللة مساهمة لكل لتر — أساس التقويم", BGRAY],
...
/* ═══ ١٧ · المستهدف بالتصنيف ═══ */
{
  const s = page("المستهدف بالتصنيف", "بالمسارين · وستة أشهر");
  const rows = SEG.map((g, i) => ({
  ...
  rows.push({ c: [{ t: "الإجمالي", b: true }, ar(T.n), ar1(N.volume / N.visits), "—",
              ...
              fill: T_BAND, b: true });
  table(s, M, 1.36, CW, [
    { t: "التصنيف", w: 16, a: "right" }, { t: "محطات", w: 7 }, { t: "التعبئة", w: 8 },
  ...
  head(s, 4.4, "الفعل التجاري لكل تصنيف");
  const rows2 = SEG.map((g, i) => ({
    ...
    fill: i % 2 ? T_NEU : W }));
  table(s, M, 4.74, CW, [
    { t: "التصنيف", w: 16, a: "right" }, { t: "المحرّك", w: 18, a: "right" },
...
/* ═══ ١٨ · أكبر الفرص ═══ */
{
  ...
  const rows = ST.slice(0, 12).map((r, i) => ({
    ...
    fill: i % 2 ? T_NEU : W }));
  table(s, M, 1.36, CW, [
    { t: "المحطة", w: 18, a: "right" }, { t: "الكود", w: 7 }, { t: "التصنيف", w: 13 },
...
/* ═══ ١٩ · مستهدف الوردية والعامل ═══ */
{
  ...
  const rows = SG.rows.slice(0, 5).map((r, i) => ({
    ...
    fill: i % 2 ? T_NEU : W }));
  table(s, M, 3.56, CW, [
    { t: "المحطة", w: 14, a: "right" }, { t: "الكود", w: 7 }, { t: "التصنيف", w: 12 },
...
/* ═══ استهداف كل منفذ ═══ */
{
  const OT = X.targeting;
  const s = page("من يستهدفه كل منفذ", "مقيس من مزيج الوقود وسلة التعبئة وساعة الذروة · "
                 + ar(OT.n) + " منفذاً");
  head(s, 1.3, "① الملف المقيس لكل تصنيف — ومن يخدمه فعلاً");
  const prof = OT.profiles.map((g, i) => ({
    c: [{ t: g.cls, a: "right", b: true }, ar(g.n), ar1(g.mlpa),
        pc0(g.diesel), ar1(g.lpv), ar(Math.round(g.vpd)), arn(g.peak + ":00"),
        pc0(g.night), { t: g.who, a: "right", fs: 10.5 }],
    fill: i % 2 ? T_NEU : W }));
  table(s, M, 1.62, CW, [
    { t: "التصنيف", w: 9, a: "right" }, { t: "منافذ", w: 7 }, { t: "MLPA", w: 8 },
    { t: "الديزل", w: 8 }, { t: "لتر/زيارة", w: 9 }, { t: "زيارة/يوم", w: 9 },
    { t: "الذروة", w: 8 }, { t: "المساء", w: 8 }, { t: "من يخدمه", w: 34, a: "right" }],
    prof, { rh: 0.36, fs: 11, hfs: 10 });

  s.addText("② وأعلى المنافذ فرصةً — ولكلٍّ استهدافه وفعله", { x: M, y: 3.78, w: CW, h: 0.3,
    fontFace: F, fontSize: 13.5, bold: true, color: ORANGE, ...rtl });
  const rows = OT.top.map((r, i) => ({
    c: [{ t: r.name.slice(0, 16), a: "right", b: true }, r.code, r.cls,
        { t: r.who, a: "right", fs: 10 }, arn(r.peak + ":00"),
        { t: r.scanned ? ar(r.comp_n) : "لم يُمسح",
          b: r.scanned, c: r.scanned ? D_BAD : INK3 },
        { t: ar(r.sar), b: true, c: D_GOOD }],
    fill: i % 2 ? T_NEU : W }));
  table(s, M, 4.1, CW, [
    { t: "المنفذ", w: 15, a: "right" }, { t: "الكود", w: 7 }, { t: "التصنيف", w: 9 },
    { t: "من يستهدفه", w: 36, a: "right" }, { t: "الذروة", w: 8 },
    { t: "منافسون ≤٥كم", w: 11 }, { t: "الفرصة ر/سنة", w: 12 }],
    rows, { rh: 0.3, fs: 10.5, hfs: 9.5 });

  const cw = (CW - 0.24) / 2;
  const bx = [rtlx(0, cw, 0.24), rtlx(1, cw, 0.24)];
  s.addShape(p.ShapeType.roundRect, { x: bx[0], y: 6.02, w: cw, h: 0.82, rectRadius: 0.05,
    fill: { color: T_BAD }, line: { color: BAD, width: 1.2 } });
  s.addText("المسح التنافسي " + ar(OT.scanned) + " منافذ من " + ar(OT.n)
    + " · الطريقة معروفة ومكرَّرة، والباقي يحتاج تشغيلها لا ابتكارها.",
    { x: bx[0] + 0.16, y: 6.12, w: cw - 0.32, h: 0.66, fontFace: F, fontSize: 10.5,
      color: INK, ...rtl });
  s.addShape(p.ShapeType.roundRect, { x: bx[1], y: 6.02, w: cw, h: 0.82, rectRadius: 0.05,
    fill: { color: T_OR }, line: { color: ORANGE, width: 1.2 } });
  s.addText(ar(OT.mismatch) + " منفذاً تصنيفُه في السجلّ يخالف سلوكه المقيس — "
    + "أشدّها العوالي MK003 مصنّفة «خط سفر» وديزلها صفر. يُراجَع التصنيف لا يُلغى.",
    { x: bx[1] + 0.16, y: 6.12, w: cw - 0.32, h: 0.66, fontFace: F, fontSize: 10.5,
      color: INK, ...rtl });
}
...
/* ═══ ٢٠ · الحافز ═══ */
{
  ...
  const flow = [["اللترات فوق الأساس", "الفارق بين إنتاج العامل وأساس ورديته", ORANGE],
                ["× هامش المساهمة", ar2(L.cpl_margin) + " هللة لكل لتر — أساس حدّي لا صافٍ", ORANGE],
  ...
  const rows = SEG.map((g, i) => ({
    ...
    fill: i % 2 ? T_NEU : W }));
  table(s, x2, 1.68, w2, [
    { t: "التصنيف", w: 34, a: "right" }, { t: "+معاملة", w: 22 },
...
/* ═══ ٢٤ · علبة المناديل ═══ */
{
  ...
  const st = [[ar(Math.round(L.box * 100)) + " هللة", "كلفة العلبة", "مؤكَّدة", ORANGE],
              ...
              [pc0(L.box / L.w_net), "من مساهمة التعبئة", "الأساس الحدّي لا الصافي", GOOD],
              [ar1(L.breakeven_extra) + " لتر", "تعادل الهدية",
               ar1(L.breakeven_sar) + " ريالاً إضافية", GOOD]];
  st.forEach((v, i) => stat(s, rtlx(i, cw, 0.18), 1.36, cw, 1.5, v[0], v[1], v[3], v[2]));
  const cw2 = (CW - 0.24) / 2;
  const bx = [rtlx(0, cw2, 0.24), rtlx(1, cw2, 0.24)];
  ...
  s.addText("متوسط فاتورتنا " + ar2(N.inv) + " ريالاً — فوق العتبة. العلبة تذهب لمن كان سيتجاوزها بلا حملة، "
    + "وتأكل " + pc0(L.box / L.w_net) + " من مساهمة تعبئته.",
...
cover("الإدارة التجارية · درب", "القرار المطلوب",
  "اعتماد المستهدف بمسارين · إقفال ثغرة البيانات قبل الإنفاق · تسعير المنتجات الثلاثة غير المسعَّرة",
