...
/* ═══ ١٩ · مستهدف الوردية والعامل ═══ */
{
  const s = page("من مستهدف المحطة إلى مستهدف العامل",
                 "ثلاث طبقات — الوحدة «معاملة» في الأوليين والريال في الثالثة · عمالة ٢٠ محطة من تقرير الربع الثاني");
  ...
  s.addText("② اختلال الطاقم على ٢٠ محطة — الطلب مقابل التوزيع",
  ...
  const rows = SG.rows.slice(0, 5).map((r, i) => ({
    c: [{ t: r.name.slice(0, 16), a: "right", b: true }, r.code, r.seg,
        ar(r.staff), ar(r.vpd), pc0(r.eve), ar1(r.half), ar1(r.want_e),
        { t: "+" + ar1(r.move), b: true, c: D_GOOD },
        { t: "+" + pc0(r.imb), b: true, c: D_BAD }],
    fill: i % 2 ? T_NEU : W }));
  table(s, M, 3.56, CW, [
    { t: "المحطة", w: 15, a: "right" }, { t: "الكود", w: 8 }, { t: "الشريحة", w: 13 },
    { t: "الطاقم", w: 8 }, { t: "معاملات/يوم", w: 11 }, { t: "حصة المساء", w: 10 },
    { t: "المسائي اليوم", w: 11 }, { t: "المطلوب", w: 9 },
    { t: "يُنقل للمساء", w: 10 }, { t: "اختلال العبء", w: 11 }],
    rows, { rh: 0.33, fs: 10.5, hfs: 9.5 });
  ...
  s.addText("٤٤٧ عاملاً في ٢٠ محطة · ١٢٫٣ عاملاً قابلون للنقل من الصباح للمساء بلا توظيف",
