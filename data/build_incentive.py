...
def scorecard(wb, idx=4):
    ...
    ws = sheet(wb, "بطاقة الأداء", [4, 26, 10, 18, 10, 12, 12, 10, 30],
               ...
               NC, idx)
    KPI = [
        ("تقييم خرائط جوجل", "نقطة", 0.10, 90, 100, "٩٠÷١٠٠=٠٫٩٠ — والملف كتبها ١٫٠٠"),
        ("مبيعات الوقود", "م٣", 0.50, 265, 250, "٢٦٥÷٢٥٠ — صحيحة في الملف"),
        ("جودة الخدمة", "٪", 0.15, 90, 90, "قياس ميداني"),
        ("عدد الشكاوى المغلقة", "عدد", 0.05, 27, 25, "كانت بلا وزن في الملف"),
        ("التدقيق الخارجي", "٪", 0.10, 90, 90, "جهة مستقلة"),
        ("التدقيق الداخلي — النظافة", "٪", 0.05, 95, 95, "داخلي"),
        ("التدقيق الداخلي — السلوك", "٪", 0.05, 90, 90, "داخلي"),
    ]   # المجموع ١٫٠٠ — وكان ٠٫٩٥ في الملف الأصلي، أي ٥٪ غير مخصَّصة
...
def audit(wb, idx=6):
    NC = 6
    ws = sheet(wb, "مراجعة الملف", [4, 30, 30, 26, 4, 40],
               "مراجعة الملف الأصلي — رقماً رقماً",
               "ما يغيّر النتيجة أولاً، ثم ما يُفسد القياس، ثم ما هو سليم", NC, idx)
    r = W.band(ws, 3, NC, "① أخطاء حسابية — تغيّر النتيجة")
    W.header(ws, r, ["#", "الموضع", "ما في الملف", "الصحيح", "", "الأثر"])
    r += 1
    ERRORS = [
        ("أيام لم تصل بياناتها",
         "٢٠–٣٠ سبتمبر مُدخَلة صفراً لا مُتروكة فارغة",
         "تُترك فارغة أو يُقسَم على أيامها",
         "التحقيق يُقرأ ٥٩٪ بدل ٩٢٫٥٪ و٩٣٫١٪ — "
         "فيسقط ١٬٠٠٠ و٢٬٥٠٠ ريال عن ١٨ عاملاً ومديرَين"),
        ("«2026 vs 2025»",
         "الشرائع −١٫٨١ · العزيزية +٠٫٩٩",
         "−٦٤٫٤٪ للشرائع",
         "القسمة معكوسة: ٢٠٢٥÷٢٠٢٦ بدل العكس — الرقم بلا معنى كما هو"),
        ("تقييم خرائط جوجل",
         "٩٠ من ١٠٠ تُحتسب ١٫٠٠",
         "٠٫٩٠",
         "يرفع بطاقة الأداء بنقطة كاملة بلا استحقاق"),
        ("عدد الشكاوى المغلقة",
         "٢٧ من ٢٥ تُحتسب ١٫٠٠",
         "١٫٠٨",
         "السقف غير متسق: المبيعات ١٫٠٦ والجودة ١٫٥٠ بلا سقف"),
    ]
    for i, (a, b, c, d) in enumerate(ERRORS, 1):
        cell(ws, r, 1, i)
        cell(ws, r, 2, a, font=W.BOLD, align=W.RGT)
        cell(ws, r, 3, b, align=W.WRAP, fill=PatternFill("solid", fgColor=B.T_BAD))
        cell(ws, r, 4, c, align=W.WRAP, fill=PatternFill("solid", fgColor=B.T_GOOD))
    ...
    r = W.band(ws, r + 1, NC, "② عيوب بنيوية — لا تغيّر رقماً اليوم لكنها تُفسد القياس")
    W.header(ws, r, ["#", "الموضع", "الحال", "ما يلزم", "", "لماذا"])
    r += 1
    FLAWS = [
        ("مجموع الأوزان", "٠٫٩٥ لا ١٫٠٠", "توزيع الـ٥٪ الناقصة",
         "النتيجة تُقسم على قاعدة ناقصة"),
        ("مؤشرات بلا وزن", "خمسة مؤشرات تُقاس ولا تدخل النتيجة",
         "وزنٌ لكلٍّ أو حذفها", "فئة المبيعات تأخذ «مبيعات الوقود» وحدها"),
        ("الشريحة الوسطى", "١٬٣٠٠ ريال عند «١٠٠٪» بالضبط",
         "حدود صريحة: ٨٥–٩٩٫٩٩ · ١٠٠–١٠٤٫٩٩ · ١٠٥ فأكثر",
         "لا تُدرَك عملياً إن كانت التالية «أكثر من ١٠٠٪»"),
        ("نسبة المدير", "٣٠٪ في الشرائع و١٥٪ في العزيزية", "معيار واحد",
         "لا سند للفارق في الملف"),
        ("متوسط ٢٠٢٥ للعزيزية", "١٠٬٠١٦ ريالاً",
         "١٥٦٬٨٩٣ — أو تُحذف السنة",
         "أشهر ٢٠٢٥ كلها أصفار إلا سبتمبر ١٬٢٥٥٬١٤٥ · والرقم لا يطابق أيّ حساب"),
        ("نطاق الترويسة", "مكتوب Jan-Aug والصف سبعة أشهر ٢٠٢٦",
         "تطابق الترويسة مع البيانات",
         "وأغسطس ٢٠٢٥ مفقود بينما سبتمبر ٢٠٢٥ موجود"),
        ("وحدات القياس", "JOD لتقييم جوجل · Ton للجودة", "نقطة · نسبة",
         "قالب منقول لم يُحوَّل، والصرف بالريال"),
    ]
    for i, (a, b, c, d) in enumerate(FLAWS, 1):
        cell(ws, r, 1, i)
        cell(ws, r, 2, a, font=W.BOLD, align=W.RGT)
        cell(ws, r, 3, b, align=W.WRAP, fill=PatternFill("solid", fgColor=B.T_GOLD))
        cell(ws, r, 4, c, align=W.WRAP)
        cell(ws, r, 5, "")
        cell(ws, r, 6, d, align=W.WRAP)
    ...
    r = W.band(ws, r + 1, NC, "③ وما هو سليم في الملف — يُقال إنصافاً")
    W.header(ws, r, ["#", "الموضع", "الحال", "الفحص", "", "النتيجة"])
    r += 1
    OK = [
        ("الميزانية اليومية الموحَّدة", "٩٬٣٧٨ و٦١٬٨٠٦ ريالاً لكل يوم",
         "تباين أيام الأسبوع", "٠٫٨٩× إلى ١٫٠٧× فقط — فالتوحيد مبرَّر ولا يحتاج تشكيلاً"),
        ("الإجمالي الإلكتروني", "مجموع ٩١ و٩٥ والديزل",
         "طوبقت المحطتان", "مطابق إلى الهللة"),
        ("الصفوف اليومية", "Act vs BU و٪ لكل يوم", "أُعيد حسابها",
         "صحيحة يوماً بيوم — الخلل في التجميع الشهري لا في اليوم"),
        ("اشتقاق الميزانية", "أغسطس × (١ + نسبة النمو)", "طوبقت المحطتان",
         "مطبَّقة بدقة: ٢٦٥٬٤٠٥×١٫٠٦ و١٬٧٣٢٬٨٨٤×١٫٠٧"),
    ]
    for i, (a, b, c, d) in enumerate(OK, 1):
        cell(ws, r, 1, i)
        cell(ws, r, 2, a, font=W.BOLD, align=W.RGT)
        cell(ws, r, 3, b, align=W.WRAP)
        cell(ws, r, 4, c, align=W.WRAP)
        cell(ws, r, 5, "")
        cell(ws, r, 6, d, align=W.WRAP, fill=PatternFill("solid", fgColor=B.T_GOOD))
        ws.row_dimensions[r].height = 34
        r += 1
    return ws
