...
الأهداف تشغيلية: المؤشرات ١ و٧ و٨ تقيس عدداً بهدف 0 (انقطاعات/مخالفات/متأخرات)،
و٢ و٣ تقيسان متوسط الأيام (تسليم ≤3 / إصدار ≤5). النتيجة ٩ مؤشرات مع حالة لكل محور.
النسخة ذات الـ١٣ محفوظة في سجل Git.
"""
import openpyxl
...
def fa_outage_count(m):  # توفر الوقود ← عدد الانقطاعات الشهرية (هدف 0)
    return f"=COUNTIFS({mrange(OUT,'B',m)})"
def fa_stock_days(m):    # توفر المواد المخزنية ← متوسط أيام التسليم (≤3)
    return f"=IFERROR(AVERAGEIFS('{ORD}'!H5:H204,{mrange(ORD,'C',m)},'{ORD}'!D5:D204,\"مواد مخزنية\"),0)"
def fa_po_days(m):       # إصدار أوامر الشراء ← متوسط أيام الإصدار (≤5)
    return f"=IFERROR(AVERAGEIFS('{ORD}'!H5:H204,{mrange(ORD,'C',m)},'{ORD}'!D5:D204,\"أمر شراء\"),0)"
...
def fa_violations(m):    # الامتثال والسلامة ← عدد المخالفات/الحوادث (هدف 0)
    return f"=COUNTIFS({mrange(TRP,'B',m)},'{TRP}'!K5:K304,\"<>لا يوجد\",'{TRP}'!K5:K304,\"<>\")"
def fa_late(m):          # التزام جدول النقل ← عدد الرحلات المتأخرة (هدف 0)
    return f"=COUNTIFS({mrange(TRP,'B',m)},'{TRP}'!J5:J304,\"*متأخر*\")"
...
KPIS_BY_AXIS = [
 ("👥 خدمة العميل", [
    (1,"توفر الوقود — عدد الانقطاعات","انقطاع",0,"0","zero","0",fa_outage_count),
    (2,"توفر المواد المخزنية — أيام التسليم","يوم",3,"≤ 3","low","0.0",fa_stock_days),
    (3,"إصدار أوامر الشراء — أيام الإصدار","يوم",5,"≤ 5","low","0.0",fa_po_days),
 ]),
 ("💰 التكاليف", [
 ...
 ("🏦 السيولة المالية", [
    (6,"أيام مخزون الوقود","يوم",3,"≤ 3","low","0.0",fa_invd),
 ]),
 ("🛡️ السلامة والامتثال", [
    (7,"الامتثال والسلامة — المخالفات/الحوادث","مخالفة",0,"0","zero","0",fa_violations),
 ]),
 ("⚙️ الكفاءة التشغيلية", [
    (8,"التزام جدول النقل — الرحلات المتأخرة","رحلة",0,"0","zero","0",fa_late),
...
def status_formula(p, a, direction):
    # هدف صفري (عدّ): ✅ إن كان الإجمالي 0، وإلا 🔴 مع العدد السنوي
    if direction == "zero":
        s = f"SUM(G{a}:R{a})"
        return f'=IF({s}=0,"✅ 0","🔴 "&TEXT({s},"0"))'
...
def main():
    ...
    # تذييل: ملاحظة الأهداف التشغيلية + الدمج
    note_r = r+1
    ws.merge_cells(f"A{note_r}:R{note_r}")
    style(ws.cell(note_r,1), None, GRAY, False, 8, "right", italic=True)
    ws.cell(note_r,1).value = ("ℹ️ أهداف تشغيلية: الانقطاعات/المخالفات/المتأخرات هدفها 0 ، وأيام "
                               "التسليم/الإصدار/المخزون حدّها أعلى. دُمجت ٤ مؤشرات متكررة في هذه التسعة.")
...
def patch_management_mapping(wb):
    ...
    if name not in wb.sheetnames:
        return
    ws = wb[name]
    mapping = ["K1 — عدد الانقطاعات (هدف 0)", "K4 — حُدِّثت طريقة الحساب",
               "K6 — مطابقة", "K2 — أيام التسليم (≤3)", "K3 — أيام الإصدار (≤5)"]
