...
// ---- STRATEGIC EXPERT LAYER ----
kids.push(new Paragraph({ children: [new PageBreak()] }));
kids.push(H1("٧.", "الطبقة الاستراتيجية الخبيرة (أربع عدسات)"));
kids.push(P("قراءة أعمق من منظور خبير استراتيجية التطبيقات، اقتصاد نقاط الولاء، تجربة العميل، ومدفوعات التطبيقات — تتجاوز ملاحظات تجربة المستخدم إلى القرارات التي تحدّد نجاح البرنامج تجارياً.", { after: 140 }));

kids.push(H2("١) استراتيجية التطبيق"));
kids.push(box("🟠 خطر «السوبر-آب»:", "التصميم طموح (محفظة + مدفوعات + مركبات + صراف + سوق + سحب). يجب تثبيت الحلقة الأساسية (تعبئة → كسب → رصيد → استبدال → رجوع) كنواة، وإبقاء ما عداها ثانوياً لتفادي تشتّت التركيز وثقل البناء.", SOFT, WARN));
kids.push(box("🟠 الخندق (Moat) غير مُستغَل:", "التطبيق يلتقط نوع السيارة والوقود والموقع والإنفاق = أصل بيانات قوي. العروض الحالية عامة لا مخصّصة؛ القيمة الحقيقية في التسويق المخصّص المبني على هذه البيانات.", SOFT, WARN));
kids.push(box("🟢 التمايز:", "التعبئة الذاتية (Pay-at-pump) + المحفظة المغلقة تمايز حقيقي عن المنافسين — يُنصح بتركيز الرسالة التسويقية عليه.", GREEN_L, GREEN));

kids.push(H2("٢) اقتصاد نقاط الولاء"));
kids.push(box("🔴 معدّل المكافأة منخفض تنافسياً:", "0.5% تقريباً على البنزين، بينما منافسون يصلون أعلى. الأذكى: كسب رفيع على البنزين (هامش ضعيف)، وكسب سخي على إنفاق التجار (هامش عالٍ ومموّل منهم) — لتحفيز السلوك دون استنزاف الهامش.", RED_L, RED));
kids.push(box("🟠 إدارة الالتزام (Liability):", "النقاط التزام في الميزانية؛ الانتهاء يديره لكن يجب نمذجة الـ Breakage بأرقام واقعية.", SOFT, WARN));
kids.push(box("🟡 إرباك مفهوم القيمة:", "خلط (نقطة / ريال / مجاني) في عرض العروض يضعف إدراك القيمة — يُعتمد أسلوب عرض واحد.", WARN_L, WARN));

kids.push(H2("٣) تجربة العميل (CX)"));
kids.push(box("🟠 زمن الوصول للقيمة:", "يجب أن يحصل العميل الجديد على أول مكافأة فوراً (هدية ترحيب + أول كسب تعبئة) — عامل حاسم في البقاء.", SOFT, WARN));
kids.push(box("🟡 توتر الأمان مقابل السهولة عند الحرق:", "QR بعدّاد + تحقق الحضور آمن لكنه احتكاك في أهم لحظة (الاستبدال) — يجب موازنته.", WARN_L, WARN));
kids.push(box("🟢 التصميم العاطفي:", "لحظات الاحتفاء (Confetti عند النجاح) حاضرة وتعزّز الارتباط.", GREEN_L, GREEN));

kids.push(H2("٤) مدفوعات التطبيق"));
kids.push(box("🔴 مسار الاسترجاع/السحب — فجوة:", "المحفظة المغلقة بلا كاش-آوت، لكن العملاء سيطلبون استرجاعاً. غياب مسار استرجاع واضح يولّد شكاوى وضغطاً تنظيمياً (ساما). يجب تعريف سياسة الاسترجاع.", RED_L, RED));
kids.push(box("🟠 محرّك التسوية مع التجار:", "الحرق عند التجار يتطلب نظام تسوية ومطابقة مالية — بند تشغيلي غائب عن التصميم.", SOFT, WARN));
kids.push(box("🟡 ملاءمة التقسيط (BNPL):", "تابي/تمارا منطقية للباقات الكبيرة لا للمشتريات الصغيرة (قهوة) — يُقصر على المبالغ الكبيرة.", WARN_L, WARN));
kids.push(box("🟡 تكلفة رافعة الشحن:", "رسوم مدى/فيزا على الشحن يجب أن تقل عن فائدة الـ Float، وإلا انقلب الشحن إلى خسارة.", WARN_L, WARN));

// ---- CONCLUSION ----
kids.push(H1("٨.", "الخلاصة"));
...
kids.push(bullet("", "استراتيجياً: ثبّتوا الحلقة الأساسية، ارفعوا الكسب على إنفاق التجار، استغلّوا بيانات المركبة للتخصيص، وعرّفوا مساري الاسترجاع والتسوية قبل الإطلاق."));
